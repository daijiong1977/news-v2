...
|---|---|---|---|---|---|
| 2026-07-08 | medium | pipeline | [news-safety-and-image-tuning](2026-07-08-news-safety-and-image-tuning.md) | Two owner-requested tunings: (1) per-dimension Stage-3 safety — kids news CAN show moderate war/politics/conflict (violence/fear/distress/adult_themes/bias reject only at ≥4), while sexual/substance/language stay strict (≥3), so hard-news sources like PBS stop being over-filtered; (2) a missing/generic (NPR facebook-default) image is CLEARED → clean category-colour card instead of the wrong NPR logo. | evaluate_rewriter_safety, per-dimension, threshold, verify_article_content, og_image, NPR, placeholder |
| 2026-07-08 | high | pipeline | [news-image-drop-starves-supply](2026-07-08-news-image-drop-starves-supply.md) | Manual full run failed: News=1 (<2) so bundle-validation refused to publish. 3 of 5 News candidates were NPR with a generic facebook-default image and got DROPPED by verify_article_content; the new safety vet then rejected 1 more. Fix: keep articles with a generic (but valid) image instead of dropping — image quality is never a reason to drop a kid-safe story. | verify_article_content, generic-social-image, news-shortfall, bundle-validation, NPR |
| 2026-07-08 | high | website + edge-fn | [magic-link-binds-clicking-device](2026-07-08-magic-link-binds-clicking-device.md) | Magic link bound the email to the device that CLICKS it, not the one that REQUESTED it — request on laptop + tap link on phone stranded the laptop's streak and re-showed onboarding silently. Fix: issue_magic_link records requester client_id; consume binds to it. Applied + live-verified. Security follow-up: token returned to anon caller + send-email-v2 open relay. | magic-link, issue_magic_link, consume_magic_link, client_id, cross-device, requester, kidsync, sign-in |
| 2026-07-08 | medium | pipeline | [filter-pickup-polish](2026-07-08-filter-pickup-polish.md) | P2 polish: asl/cd9 false-positive patterns dropped; unreachable no-og:image branch fixed (honest telemetry); burst-aware cadence estimator + ±2/run clamp; html_list titles fall back to img-alt/slug (DOGOnews); active_weekdays + is_backup finally honored; −97 lines dead code. | forbidden_filter, og-image, cadence, burst, scraper, html_list, active_weekdays, is_backup, dead-code |
| 2026-07-08 | high | pipeline | [safety-quality](2026-07-08-safety-quality.md) | Safety gate was self-graded (rewriter scored its own output; no independent reviewer saw the rewritten text); body-only self-harm passed every gate (forbidden regex only scanned RSS metadata); the easy-body prompt band (170-210w) sat BELOW the QA gate (200-320w), manufacturing the 66 body_too_short tickets. Fix: independent vet call + is_forbidden on rewritten bodies + bands aligned & measured at gen time. | independent-vet, self-grading, filter_safe_rewrites, is_forbidden, body_too_short, WC_BANDS |
...
| 2026-07-11 | [same-story-shipped-twice](2026-07-11-same-story-shipped-twice.md) | pipeline | Two sources' version of one wire story both shipped: cluster_id never enforced + subject-cap only guarded top 3; fix = deterministic same-story dedup (cluster/title/subject) + promotion title-guard | fixed |
| 2026-07-11 | [deep-dig-backfill](2026-07-11-deep-dig-backfill.md) | pipeline | Short category jumped straight to yesterday's content; fix = reactive deep-dig into items 5..15 of today's same feeds before carry-over | fixed |
| 2026-08-25 | [mega-curator-flash-cot-truncation](2026-08-25-mega-curator-flash-cot-truncation.md) | pipeline | Daily run crashed: V4 Flash CoT blew the 20k max_tokens budget at 30 candidates (3rd truncation bump); fix = 20k→64k, next time = split-batch | fixed |
| 2026-09-05 | [enrich-flash-cot-truncation-week-outage](2026-09-05-enrich-flash-cot-truncation-week-outage.md) | pipeline | Week of failed publishes 08-29..09-05: enrich calls truncated at 20k on V4 Flash CoT → empty questions → bundle validation refused; fix = bump enrich/vet/default budgets to 64k | fixed |
