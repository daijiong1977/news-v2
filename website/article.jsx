...
// ——————— PRINTABLE PDF (4 pages: Read · Background · Quiz · Think) ———————
// Builds a self-contained printable HTML, opens it in a new tab, auto-
// triggers the browser's print dialog so the kid can "Save as PDF". Pure
// browser-native — no external libraries, works offline once saved.
function escHtml(s) {
...
function buildPrintableHTML(article, paragraphs) {
  const cat = article.category || 'News';
  const catColors = { News:'#ff6b5b', Science:'#17b3a6', Fun:'#9061f9' };
  const catColor = catColors[cat] || '#1b1230';
  const today = new Date().toLocaleDateString(undefined, { weekday:'long', month:'short', day:'numeric', year:'numeric' });
  const minedAt = article.minedAt ? new Date(article.minedAt).toLocaleDateString(undefined, {month:'short', day:'numeric', year:'numeric'}) : '';

  // Article body — split into paragraphs
  const bodyHtml = paragraphs.map(p => `<p>${escHtml(p)}</p>`).join('');

  // Word Treasure
  const kwHtml = (article.keywords || []).map(k =>
    `<div class="kw"><strong>${escHtml(k.term)}</strong> — ${escHtml(k.def)}</div>`
  ).join('');

  // Background paragraphs
  const bg = article.backgroundRead || '';
  const bgHtml = bg.split(/\n\n+/).filter(Boolean).map(p => `<p>${escHtml(p.trim())}</p>`).join('');

  // Article structure (5W or mind-tree)
  const structHtml = (article.articleStructure || []).map(line => {
    const m = String(line).match(/^(\s*)(.*)$/);
    const indent = m ? m[1].length : 0;
    return `<div class="struct" style="padding-left:${indent * 8}px">${escHtml(m ? m[2] : line)}</div>`;
  }).join('');
  ...
  const quizHtml = quiz.map((q, i) => {
    const opts = (q.options || []).map((opt, oi) => {
      ...
      return `<div class="opt"><span class="bubble">${letter}</span><span>${escHtml(opt)}</span></div>`;
    }).join('');
    return `<div class="qa"><div class="q-num">${i + 1}.</div><div class="q-body"><div class="q-text">${escHtml(q.q)}</div>${opts}</div></div>`;
  }).join('');
  const answerKey = quiz.map((q, i) => {
    const correct = (q.options || [])[q.a];
    const letter = String.fromCharCode(65 + (q.a || 0));
    return `${i + 1}: ${letter}`;
  }).join(' · ');

  // Think & Share — discussion questions + ruled lines
  const discussionHtml = (article.discussion || []).map((d, i) =>
    `<div class="dq"><div class="dq-num">${i + 1}.</div><div class="dq-text">${escHtml(d)}</div></div>`
  ).join('');
  const ruledLines = Array.from({length: 14}, () => `<div class="rule"></div>`).join('');

  return `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8"/>
<title>${escHtml(article.title)} · News Oh,Ye!</title>
<link href="https://fonts.googleapis.com/css2?family=Fraunces:wght@700;800;900&family=Nunito:wght@400;600;700;800&display=swap" rel="stylesheet">
<style>
  @page { size: Letter; margin: 0.5in; }
  * { box-sizing: border-box; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
  body {
    font-family: 'Nunito', sans-serif; color: #1b1230;
    font-size: 12pt; line-height: 1.5; margin: 0;
  }
  .page { padding: 0.25in 0; min-height: calc(100vh - 1in); page-break-after: always; }
  ...
  .cat-tag {
    background: ${catColor}; color: #fff; padding: 3px 10px;
    border-radius: 999px; font-size: 9pt; font-weight: 800;
    text-transform: uppercase; letter-spacing: .08em;
  }
  .step-label {
    font-size: 9pt; font-weight: 800; color: #6b5c80;
    letter-spacing: .12em; text-transform: uppercase;
  }
  .footer { margin-top: 18px; padding-top: 8px; border-top: 1px dashed #d0c4b4;
    display: flex; justify-content: space-between; font-size: 8.5pt; color: #9a8d7a; }
  h1 { font-family: 'Fraunces', serif; font-weight: 900; font-size: 22pt;
       line-height: 1.1; margin: 6px 0 8px; }
  h2 { font-family: 'Fraunces', serif; font-weight: 800; font-size: 14pt;
       margin: 18px 0 8px; color: #1b1230; }
  h3 { font-family: 'Fraunces', serif; font-weight: 800; font-size: 11pt;
       margin: 10px 0 6px; color: ${catColor}; }
  p { margin: 0 0 8px; }

  /* Page 1: Read & Words */
  .meta { color: #6b5c80; font-size: 9pt; font-weight: 700; margin-bottom: 12px; }
  .body p:first-of-type::first-letter {
    font-family:'Fraunces', serif; font-weight: 900; font-size: 36pt;
    float: left; line-height: 0.85; margin: 4px 6px 0 0; color: ${catColor};
  }
  .kw { padding: 5px 10px; background: #fff9ef; border-left: 3px solid ${catColor};
        margin-bottom: 5px; font-size: 10.5pt; }
  .kw strong { color: ${catColor}; font-weight: 800; }

  /* Page 2: Background */
  .bg p { background: #fff9ef; padding: 10px 14px; border-radius: 6px;
          margin-bottom: 8px; }
  .struct { font-size: 10.5pt; margin: 3px 0; line-height: 1.4;
            white-space: pre-wrap; }

  /* Page 3: Quiz */
  .qa { display: flex; gap: 10px; margin-bottom: 14px;
        page-break-inside: avoid; }
  .q-num { font-family:'Fraunces', serif; font-weight: 900; font-size: 14pt;
           color: ${catColor}; min-width: 22px; }
  .q-body { flex: 1; }
  .q-text { font-weight: 700; font-size: 11.5pt; margin-bottom: 6px; }
  .opt { display: flex; align-items: center; gap: 8px; padding: 3px 0; font-size: 10.5pt; }
  .bubble { display: inline-flex; align-items: center; justify-content: center;
            width: 18px; height: 18px; border: 1.5px solid #1b1230;
            border-radius: 50%; font-weight: 800; font-size: 9pt; flex-shrink: 0; }
  .answer-key { margin-top: 16px; padding: 10px 14px;
                background: #f0ebff; border-radius: 8px; font-size: 9pt;
                color: #6b5c80; font-weight: 600; }
  .answer-key strong { color: #9061f9; }

  /* Page 4: Think & Share */
  .dq { display: flex; gap: 10px; padding: 10px 12px;
        background: #fff4c2; border-radius: 8px; border: 1.5px solid #ffe28a;
        margin-bottom: 10px; }
  .dq-num { font-family:'Fraunces', serif; font-weight: 900; font-size: 14pt; color: #8a6d00; }
  .dq-text { font-family: 'Fraunces', serif; font-weight: 700; font-size: 11.5pt;
             line-height: 1.3; flex: 1; }
  .write-here { font-size: 10pt; font-weight: 700; color: #6b5c80;
                letter-spacing: .08em; text-transform: uppercase; margin: 14px 0 8px; }
  .rule { border-bottom: 1px solid #d0c4b4; height: 22px; }

  @media screen {
    body { background: #f0e8d8; padding: 20px; }
    .page { background: #fff; box-shadow: 0 4px 20px rgba(0,0,0,0.1);
            margin: 0 auto 20px; max-width: 8.5in; padding: 0.5in; }
    .print-banner { position: fixed; top: 16px; right: 16px;
                    background: #1b1230; color: #ffc83d; padding: 10px 16px;
                    border-radius: 999px; font-weight: 800; font-size: 12pt;
...
<body>
<button class="print-banner" onclick="window.print()">🖨 Print / Save as PDF</button>

<!-- ——— Page 1: Read & Words ——— -->
<div class="page">
  <div class="header">
    <span class="cat-tag">${escHtml(cat)}</span>
    <span class="step-label">Step 1 · Read &amp; Words</span>
    <div style="flex:1"></div>
    <span class="meta">${escHtml(today)}</span>
  </div>
  <h1>${escHtml(article.title)}</h1>
  <div class="meta">📰 ${escHtml(article.source || '')}${minedAt ? ' · 🗞 Mined ' + escHtml(minedAt) : ''} · ⏱ ${escHtml(String(article.readMins || 5))} min read</div>
  <div class="body">${bodyHtml}</div>
  ${kwHtml ? '<h3>🔑 Word Treasure</h3>' + kwHtml : ''}
  <div class="footer"><span>News Oh,Ye! · Step 1 of 4</span><span>${escHtml(today)}</span></div>
</div>
...
<div class="page">
  <div class="header">
    <span class="cat-tag">${escHtml(cat)}</span>
    <span class="step-label">Step 2 · Background &amp; Structure</span>
    <div style="flex:1"></div>
    <span class="meta">${escHtml(article.title.slice(0, 50))}…</span>
  </div>
  ${bgHtml ? '<h2>🧭 Background you need</h2><div class="bg">' + bgHtml + '</div>' : ''}
  ${structHtml ? '<h2>🔍 Break it down</h2>' + structHtml : ''}
  ${article.whyItMatters ? '<h2>💡 Why it matters</h2><p>' + escHtml(article.whyItMatters) + '</p>' : ''}
  <div class="footer"><span>News Oh,Ye! · Step 2 of 4</span><span>${escHtml(today)}</span></div>
</div>
...
<div class="page">
  <div class="header">
    <span class="cat-tag">${escHtml(cat)}</span>
    <span class="step-label">Step 3 · Quiz</span>
    <div style="flex:1"></div>
    <span class="meta">${escHtml(article.title.slice(0, 50))}…</span>
  </div>
  <h2>🎯 Test what you remember</h2>
  ${quizHtml || '<p>No quiz available for this article.</p>'}
  ${answerKey ? '<div class="answer-key"><strong>Answer key</strong> (try the quiz first!) — ' + escHtml(answerKey) + '</div>' : ''}
  <div class="footer"><span>News Oh,Ye! · Step 3 of 4</span><span>${escHtml(today)}</span></div>
</div>
...
<div class="page">
  <div class="header">
    <span class="cat-tag">${escHtml(cat)}</span>
    <span class="step-label">Step 4 · Think &amp; Share</span>
    <div style="flex:1"></div>
    <span class="meta">${escHtml(article.title.slice(0, 50))}…</span>
  </div>
  <h2>💭 Questions to think about</h2>
  ${discussionHtml || '<p>No discussion questions for this article.</p>'}
  <div class="write-here">My thoughts (at least 20 words):</div>
  ${ruledLines}
