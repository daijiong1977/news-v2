...
def main() -> None:
    ...
    log.info("Sources: %s", [s.name for s in enabled])
    ...
    while dup_rounds < 2:
        ...
        log.info("Dup check: verdict=%s", dup_result.get("verdict"))
...
<title>Fun Today — {today_date}</title>
<style>
  ...
  .topic{{background:#f5e6ff;color:#704494;border-radius:6px;padding:6px 12px;display:inline-block;font-weight:700;font-size:14px;margin-left:6px;}}
  .stats{{background:#f6f6f6;border-radius:6px;padding:10px 14px;margin:12px 0;font-size:13px;color:#555;}}
  .story{{background:#fdf6ff;border:2px solid #c49fe8;border-radius:10px;padding:22px 26px;margin:28px 0;box-shadow:0 2px 6px rgba(0,0,0,.06);}}
  ...
  .story .src{{font-size:12px;color:#704494;margin-bottom:6px;}}
  .variant{{background:#fff;border-left:4px solid #c49fe8;padding:14px 18px;margin:14px 0;border-radius:4px;}}
  .variant h3{{color:#1b1230;margin-top:0;font-family:Georgia,serif;font-weight:800;font-size:20px;}}
  .variant .label{{display:inline-block;background:#c49fe8;color:#fff;padding:2px 10px;border-radius:10px;font-size:11px;font-weight:700;letter-spacing:.05em;text-transform:uppercase;margin-bottom:8px;}}
  .variant .label.middle{{background:#9061f9;}}
...
<body>
<h1>🎈 Fun Today — {today_date} <span class="topic">{esc(topic)}</span></h1>
<div class="meta">Weekday topic · 3 sources per day · BBC Tennis 3x/week (Tue/Sat/Sun).</div>
...
"""]
    ...
    for i, w in enumerate(winners):
        ...
        parts.append(f"""
<div class="story">
  <div class="src">🎈 <b>{esc(src.name)}</b> · <a href="{esc(src_url)}" target="_blank">{esc(src_art.get('title',''))}</a> · {esc(src_host)} · picked slot: {esc(w.get('winner_slot',''))} · {vet_pills}</div>
  <div class="hero">
...
{bp}
  ...
  <details><summary>Full source body</summary><pre>""" + esc(src_art.get("body", "")) + """</pre></details>
    ...
    (today_dir / "fun_today.json").write_text(json.dumps({
    ...
    html_path = Path("/Users/jiong/myprojects/news-v2/website/fun-today.html")
    html_path.write_text("".join(parts))
    log.info("HTML: %s", html_path)
    log.info("View: http://localhost:18100/fun-today.html")
