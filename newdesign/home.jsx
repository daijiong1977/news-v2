...
function HomePage({ onOpen, onOpenArchive, level, setLevel, cat, setCat, progress, theme, heroVariant, tweaks, onOpenUserPanel, archiveDay }) {
  ...
  const effectiveLevel = isZh ? 'Sprout' : level;
  const matchesLevel = (a) => a.level === effectiveLevel;
  const isArchive = typeof archiveDay === 'number' && archiveDay > 0;

  const todayArticles = useMemoH(() => ARTICLES.filter(a => (a.day||0) === 0 && matchesLevel(a)), [effectiveLevel]);
  const archiveArticles = useMemoH(() => ARTICLES.filter(a => (a.day||0) === archiveDay && matchesLevel(a)), [effectiveLevel, archiveDay]);
      ...
      <section style={{maxWidth:1180, margin: isArchive ? '24px auto 0' : '32px auto 0', padding:'0 28px', position:'relative'}}>
        <div style={{display:'flex', gap:12, flexWrap:'wrap', alignItems:'center'}}>
          ...
          <span style={{fontSize:13, color:'#7a6b8c', fontWeight:600}}>
            {isZh ? (<>Reading in <b style={{color:'#1b1230'}}>中文</b> · <b>Sprout</b> level</>) : (<>Showing stories at <b style={{color:'#1b1230'}}>{level}</b> level</>)}
          </span>
        </div>
        {calendarOpen && (
          ...
          return (
            <button key={a.id} onClick={()=>onOpenArticle(a.id)} style={{
              ...
              <div style={{flex:1, minWidth:0}}>
                ...
                <div style={{fontSize:11, color:'#6b5c80', fontWeight:600, marginTop:2}}>{a.category} · {a.level} · {a.readMins} min</div>
              </div>
